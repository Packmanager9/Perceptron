...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    let xrun = 0
    let yrun = 0
    let redval = Math.random()
    let greenval = Math.random()
    let blueval = Math.random()

    let meshes = []
    for (let t = 0; t < 1000; t++) {

        let SandMesh = new GenNN([redval, blueval, greenval], 5, [243, 81, 27, 9, 3], 4)

        meshes.push()
    }

    // let SandMesh2 = new GenNN([.5,.5,.5,.5], 5, [128, 64, 32, 16, 8], 4)
