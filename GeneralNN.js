...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class GeneralizableNeuralNetwork {
        constructor(inputs, layers, layersetup, outputs) {
            this.inputs = inputs
            this.layersetup = [...layersetup]
            this.outputs = outputs
            this.layersstorage = layers
            this.layers = []
            this.layercount = layers.length
            this.layerlocal = []
            // this.layersetup.unshift(0)
            ...
            for (let t = 0; t < this.layercount; t++) {
                ...
                if (t == 0) {
                    ...
                    //     node.push(this.weights())
                    // }

                    for (let f = 0; f < this.layersetup[this.layers.length]; f++) {
                        node.push(this.weights())
                        satori.push(0)
                    }
                    ...
                    for (let f = 0; f < this.layersetup[this.layers.length]; f++) {
                        for (let n = 0; n < this.layersetup[this.layers.length - 1]; n++) {
                            node.push(this.weights())
                            satori.push(0)
                        }
                ...
                // zen.splice(1)
                this.layerlocal.push(zen)
            }
            for (let t = 0; t < this.outputs.length; t++) {
                for (let k = 0; k < this.layersetup[this.layersetup.length - 1]; k++) {
                    let node = []
                    node.push(this.weights())
                }
            }
        ...
        compute() {
            // for (let k = 0; k < this.layers[0].length; k++) {
            //     for (let t = 0; t < this.inputs.length; t++) {
            //         this.layerlocal[0][k][t] = 0
            //     }
            // }

            //     for(let g = 0;g<this.layers.length;g++){
            //     for (let k = 0; k < this.layers[g].length; k++) {
            //         for (let t = 0; t < this.inputs.length; t++) {
            //             this.layerlocal[g][k][t] += this.inputs[t] * this.layers[g][k][t]
            //         }
            //     }
            // }
            for (let g = 0; g < this.layers.length; g++) {
                if (g == 0) {
                    for (let k = 0; k < this.layers[g].length; k++) {
                        for (let t = 0; t < this.inputs.length; t++) {
                            this.layerlocal[g][k][t] = 0
                        }
                    }
                } else {
                    for (let k = 0; k < this.layers[g - 1].length; k++) {
                        for (let t = 0; t < this.layerlocal[g][k].length; t++) {
                            this.layerlocal[g][k][t] = 0
                        }
                    }
                }
            ...
            for (let g = 0; g < this.layers.length; g++) {
                if (g == 0) {
                    for (let k = 0; k < this.layers[g].length; k++) {
                        for (let t = 0; t < this.inputs.length; t++) {
                            for (let h = 0; h < this.layerlocal[g][k].length; h++) {
                                this.layerlocal[g][k][h] += this.inputs[t] * this.layers[g][k][h]
                            }
                        }
                    }
                } else {
                    for (let k = 0; k < this.layers[g].length; k++) {
                        for (let t = 0; t < this.layerlocal[g][k].length; t++) {
                            for (let n = 0; n < this.layerlocal[g - 1].length; n++) {
                                // console.log(this.layerlocal[g - 1][k], k)
                                for (let w = 0; w < this.layerlocal[g - 1][k].length; w++) {
                                    this.layerlocal[g][k][t] += this.layerlocal[g - 1][n][w] * this.layers[g][k][t]
                                }
                            }
                        }
                    }
    ...
    for(let t =0;t<25;t++){
        let SandMesh = new GenNN([(dot.x - 350) / 350, (dot.y - 350) / 350], 3, [2, 2, 4], 4)
