...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            if (box1.isPointInside(TIP_engine)) {
                ...
                for (let t = 0; dots.length < 1001; t++) {
                    let dot = new Circle(Math.random() * 700, Math.random() * 199, 3, "black")
                    let colorpick = Math.floor(Math.random() * 3.00)
    ...
    class Perceptron {
        ...
        compute() {
            ...
            let redNorm = this.totalRed

            let blueNorm = this.totalBlue

            let greenNorm = this.totalGreen
            ...
            redNorm = this.normalize(redNorm, Math.min(Math.min(this.totalBlue, this.totalGreen), this.totalRed), Math.max(Math.max(this.totalBlue, this.totalGreen), this.totalRed))
            blueNorm = this.normalize(blueNorm, Math.min(Math.min(this.totalBlue, this.totalGreen), this.totalRed), Math.max(Math.max(this.totalBlue, this.totalGreen), this.totalRed))
            greenNorm = this.normalize(greenNorm, Math.min(Math.min(this.totalBlue, this.totalGreen), this.totalRed), Math.max(Math.max(this.totalBlue, this.totalGreen), this.totalRed))
