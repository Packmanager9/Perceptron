...
window.addEventListener('DOMContentLoaded', (event) => {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            if(box1.isPointInside(TIP_engine)){
                for(let t = 0;t<cepts.length;t++){
                    if(cepts[t].totalLeft < cepts[t].totalRight){
                        cepts[t].marked = 1 
                    }
                }
            }
    ...
    for (let t = 0;dots.length<99; t++) {
        let dot = new Circle(Math.random() * 700, Math.random() * 199, 3, "red")
    ...
    let box1 = new Rectangle(0, 0, 350, 200, "white")

    let box2 = new Rectangle(350, 0, 350, 200, "white")
    ...
    class Perceptron {
        constructor() {
            this.inputNode1 = new Circle(200, 300, 10, "white")
            this.inputNode2 = new Circle(500, 300, 10, "white")
            this.marked = 0
            this.brainNodes = []
            ...
            for (let t = 0; t < 4; t++) {
                ...
                this.brainNodes.push([node, weight1, weight2])
            }



            for (let t = 0; t < 2; t++) {
                let node = new Circle(200 + (t * 300), 620, 12, "white")
                let weight1 = Math.random()
                let weight2 = Math.random()
        ...
        draw() {
            ...
            for (let t = 0; t < dots.length; t++) {
                if (box1.isPointInside(dots[t])) {
                    this.leftcount++
                }
                if (box2.isPointInside(dots[t])) {
                    this.rightcount++
                }
            }

            canvas_context.fillText(`${this.leftcount}`,50, 250)
            canvas_context.fillText(`${this.rightcount}`,650, 250)



            this.inputNode1.color = `rgba(255,255,255,${(this.leftcount / dots.length)/cepts.length})`
            this.inputNode2.color = `rgba(255,255,255,${(this.rightcount / dots.length)/cepts.length})`
        ...
        compute() {
            ...
            for (let t = 0; t < this.outputs.length; t++) {
                for (let k = 0; k < this.brainNodes.length; k++) {
                    weightCalc1 += this.brainNodes[k][1] * (this.leftcount / dots.length)
                    weightCalc2 += this.brainNodes[k][2] * (this.rightcount / dots.length)
                    weightCalc1 *= this.outputs[t][k + 1]
                    weightCalc2 *= this.outputs[t][k + 1]

                }
                this.totalLeft += weightCalc1
                this.totalRight += weightCalc2
            }

            this.totalLeft = Math.max(this.totalLeft, 0)
            this.totalRight = Math.max(this.totalRight, 0)


            this.totalLeft = (this.totalLeft/(this.totalLeft+this.totalRight))

            this.totalRight = (this.totalRight/(this.totalLeft+this.totalRight))
            ...
            this.outputs[0][0].color = `rgba(255,255,255,${this.totalLeft/cepts.length})`
            this.outputs[1][0].color = `rgba(255,255,255,${this.totalRight/cepts.length})`
